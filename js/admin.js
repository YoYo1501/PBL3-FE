...
function bindPaginationControls() {
    bindPaginationControl("registrations", renderRegistrationsList);
    bindPaginationControl("requests", renderRequestsList);
    bindPaginationControl("transfers", renderTransfersList);
    bindPaginationControl("renewals", renderRenewalsList);
    ...
    bindPaginationControl("revenue", renderRevenueTable);
    bindPaginationControl("notifications", loadNotifications);
}

function bindPaginationControl(key, renderFn) {
...
function applyServerPagination(key, data) {
    ...
    state.totalItems = typeof data?.totalItems === "number" ? data.totalItems : items.length;
    return items;
}
...
function bindReloadButtons() {
    ...
    document.getElementById("reload-contracts-btn")?.addEventListener("click", loadContracts);
    document.getElementById("reload-invoices-btn")?.addEventListener("click", loadInvoices);
    document.getElementById("reload-rooms-btn")?.addEventListener("click", loadRooms);
    document.getElementById("reload-students-btn")?.addEventListener("click", loadStudents);
    document.getElementById("reload-notifications-btn")?.addEventListener("click", loadNotifications);
}

function getContractFilters() {
    return {
        keyword: document.getElementById("contract-search")?.value.trim().toLowerCase() || "",
...
function bindContractControls() {
    ...
    document.getElementById("contract-search")?.addEventListener("input", rerenderContracts);
    document.getElementById("contract-filter-status")?.addEventListener("change", rerenderContracts);
    document.getElementById("contracts-prev-btn")?.addEventListener("click", () => {
        if (contractPage <= 1) return;
        contractPage -= 1;
        loadContracts();
    });
    document.getElementById("contracts-next-btn")?.addEventListener("click", () => {
        contractPage += 1;
        loadContracts();
    });

    document.getElementById("contract-edit-form")?.addEventListener("submit", async event => {
    ...
    document.getElementById("deactivate-contract-btn")?.addEventListener("click", async () => {
        if (!selectedContractId) {
        ...
        const res = await callApi(`/contracts/${selectedContractId}`, { method: "DELETE" });
        if (res?.ok) {
            ...
            setContractError(res?.data?.message || "Không thể vô hiệu hóa hợp đồng.");
        }
    });
}
...
function getInvoiceFilters() {
    return {
        period: document.getElementById("invoice-filter-period")?.value.trim() || "",
        status: document.getElementById("invoice-filter-status")?.value || ""
    };
}

function getActiveInvoicePeriod() {
    const filterPeriod = document.getElementById("invoice-filter-period")?.value.trim();
    if (filterPeriod) return filterPeriod;
    return document.getElementById("invoice-generate-period")?.value.trim()
...
function bindInvoiceControls() {
    ...
    importForm?.addEventListener("submit", async event => {
        ...
        const period = document.getElementById("invoice-import-period").value.trim();
        ...
        const res = await callApiUpload(`/invoices/import?period=${encodeURIComponent(period)}`, formData);
        if (res?.ok) {
            adminToast(res.data?.message || "Đã import dữ liệu điện nước.");
            document.getElementById("invoice-filter-period").value = period;
            setInvoiceActionError("");
        } else {
            errorEl.textContent = res?.data?.message || "Không thể import file Excel.";
        }
    });
    ...
    generateForm?.addEventListener("submit", async event => {
        ...
        const period = document.getElementById("invoice-generate-period").value.trim();
        const electricPricePerKwh = Number(document.getElementById("invoice-electric-price").value);
        const waterPricePerM3 = Number(document.getElementById("invoice-water-price").value);
        ...
        if (res?.ok) {
            adminToast(res.data?.message || "Da tao hoa don nhap.");
            document.getElementById("invoice-filter-period").value = period;
            loadInvoices();
        } else {
    ...
    document.getElementById("invoice-filter-period")?.addEventListener("input", reloadFilteredInvoices);
    document.getElementById("invoice-filter-status")?.addEventListener("change", reloadFilteredInvoices);

    document.getElementById("publish-invoices-btn")?.addEventListener("click", async () => {
        ...
        const res = await callApi(`/invoices/publish?period=${encodeURIComponent(period)}`, { method: "POST" });
        if (res?.ok) {
            adminToast(res.data?.message || "Da phat hanh hoa don.");
            setInvoiceActionError("");
            loadInvoices();
        } else {
            setInvoiceActionError(res?.data?.message || "Không thể phát hành hóa đơn.");
        }
    });

    document.getElementById("remind-debt-btn")?.addEventListener("click", async () => {
        const period = getActiveInvoicePeriod();
        const query = period ? `?period=${encodeURIComponent(period)}` : "";
        const res = await callApi(`/invoices/remind-debt${query}`, { method: "POST" });
        if (res?.ok) {
            adminToast(res.data?.message || "Da gui nhac no.");
            setInvoiceActionError("");
        } else {
    ...
    document.getElementById("export-invoices-btn")?.addEventListener("click", async () => {
        const period = getActiveInvoicePeriod();
        if (!period) {
        ...
        const res = await callApiBlob(`/invoices/export?period=${encodeURIComponent(period)}`);
        if (!res?.ok || !res.blob) {
        ...
        adminToast("Da xuat file Excel hoa don.");
        setInvoiceActionError("");
    });
}
...
function summarizeRooms(rooms) {
    ...
    return [
        { title: "Phong con cho", value: `${available}`, description: "Cac phong con the nhan them sinh vien." },
        { title: "Phong da day", value: `${full}`, description: "Can theo doi de can doi khi co yeu cau chuyen." },
        { title: "Phong nam", value: `${male}`, description: "Tong so phong dang danh cho sinh vien nam." },
        { title: "Phong nu", value: `${female}`, description: "Tong so phong dang danh cho sinh vien nu." }
    ];
}
...
async function loadRegistrations() {
    ...
    adminRegistrations = Array.isArray(res?.data) ? res.data : [];
...
function renderRegistrationsList() {
    ...
    if (!container) return;
    updatePaginationUi("registrations", adminRegistrations.length);
    ...
    const { items } = getPagedItems("registrations", adminRegistrations);
...
async function loadRequests() {
    ...
    adminRequests = Array.isArray(res?.data) ? res.data : [];
    resetPage("requests");
    renderRequestsList();
}
...
function renderRequestsList() {
    ...
    if (!container) return;
    updatePaginationUi("requests", adminRequests.length);
    ...
    const { items } = getPagedItems("requests", adminRequests);
    container.innerHTML = items.map(item => `
        <article class="queue-item">
            <div class="queue-head">
                <strong>${escapeHtml(item.title)}</strong>
...
async function loadTransfers() {
    ...
    adminTransfers = Array.isArray(res?.data) ? res.data : [];
    resetPage("transfers");
    renderTransfersList();
}
...
function renderTransfersList() {
    ...
    if (!container) return;
    updatePaginationUi("transfers", adminTransfers.length);
    ...
    const { items } = getPagedItems("transfers", adminTransfers);
    container.innerHTML = items.map(item => `
        <article class="queue-item">
            <div class="queue-head">
    ...
    container.querySelectorAll("[data-transfer-reject]").forEach(button => {
        button.addEventListener("click", () => withAction(button, async () => {
            ...
            const resReject = await callApi(`/roomtransfers/${button.dataset.transferReject}/approve`, {
                method: "PUT",
                body: JSON.stringify({ isApproved: false, rejectionReason: reason || "Khong phu hop dieu kien chuyen phong" })
            });
            if (resReject?.ok) {
...
async function loadRenewals() {
    ...
    adminRenewals = Array.isArray(res?.data) ? res.data : [];
    resetPage("renewals");
    renderRenewalsList();
}
...
function renderRenewalsList() {
    ...
    if (!container) return;
    updatePaginationUi("renewals", adminRenewals.length);

    if (!adminRenewals.length) {
        container.innerHTML = "<div class=\"empty-state\">Không có yêu cầu gia hạn nào đang chờ.</div>";
        return;
    }

    const { items } = getPagedItems("renewals", adminRenewals);
    container.innerHTML = items.map(item => `
        <article class="queue-item">
            <div class="queue-head">
    ...
    container.querySelectorAll("[data-renewal-reject]").forEach(button => {
        button.addEventListener("click", () => withAction(button, async () => {
            ...
            const resReject = await callApi(`/contracts/renewals/${button.dataset.renewalReject}/approve`, {
                method: "PUT",
                body: JSON.stringify({ isApproved: false, rejectionReason: reason || "Chua du dieu kien gia han" })
            });
            if (resReject?.ok) {
...
async function loadContracts() {
    ...
    tbody.innerHTML = "<tr><td colspan=\"7\" class=\"table-empty\">Đang tải danh sách hợp đồng...</td></tr>";
    ...
    if (selectedContractId) {
        const exists = adminContracts.some(contract => contract.id === selectedContractId);
        if (exists) {
...
function renderContractsTable() {
    ...
    const totalPages = Math.max(1, Math.ceil(contractTotalItems / CONTRACTS_PAGE_SIZE));
    ...
    if (!adminContracts.length) {
        tbody.innerHTML = "<tr><td colspan=\"7\" class=\"table-empty\">Không có hợp đồng phù hợp bộ lọc hiện tại.</td></tr>";
        return;
    }

    tbody.innerHTML = adminContracts.map(contract => `
    ...
    tbody.querySelectorAll("[data-contract-view]").forEach(button => {
        button.addEventListener("click", () => selectContract(Number(button.dataset.contractView)));
    });
}
...
async function selectContract(contractId) {
    ...
    document.getElementById("contract-detail-code").textContent = contract.contractCode || "Da chon";
    document.getElementById("contract-detail-student").textContent = contract.studentName || "-";
    document.getElementById("contract-detail-room").textContent = `${contract.roomCode || "-"} (${contract.roomType || "-"})`;
    document.getElementById("contract-detail-days").textContent = `${contract.daysRemaining ?? 0} ngay`;
    document.getElementById("contract-detail-renew").textContent = contract.canRenew ? "Co the gia han" : "Chua den han gia han";

    document.getElementById("contract-start-date").value = toDateInputValue(contract.startDate);
    document.getElementById("contract-end-date").value = toDateInputValue(contract.endDate);
...
function getRoomFilters() {
    return {
        keyword: document.getElementById("room-search")?.value.trim().toLowerCase() || "",
...
function bindRoomControls() {
    ...
    document.getElementById("room-search")?.addEventListener("input", rerenderRooms);
    document.getElementById("room-filter-status")?.addEventListener("change", rerenderRooms);
    ...
    document.getElementById("room-form")?.addEventListener("submit", async event => {
        event.preventDefault();
        const buildingId = Number(document.getElementById("room-building-id").value);
        ...
        const currentOccupancy = Number(document.getElementById("room-occupancy").value);
        ...
        if (res?.ok) {
            adminToast(res.data?.message || (selectedRoomId ? "Da cap nhat phong." : "Da tao phong."));
            setRoomError("");
            await loadRooms();
            if (res.data?.data?.id) {
    ...
    document.getElementById("delete-room-btn")?.addEventListener("click", async () => {
        ...
        const res = await callApi(`/room/${selectedRoomId}`, { method: "DELETE" });
        if (res?.ok) {
    ...
    document.getElementById("facility-form")?.addEventListener("submit", async event => {
        ...
        const quantity = Number(document.getElementById("facility-quantity").value);
        ...
        if (selectedFacilityId) {
            ...
            res = await callApi("/facilities", {
                method: "POST",
                body: JSON.stringify({ roomId: selectedRoomId, name, quantity, status })
            });
        }
    ...
    document.getElementById("delete-facility-btn")?.addEventListener("click", async () => {
        if (!selectedFacilityId) {
        ...
        const res = await callApi(`/facilities/${selectedFacilityId}`, { method: "DELETE" });
        if (res?.ok) {
...
function populateRoomBuildings() {
    ...
    select.innerHTML = roomBuildings.length
        ? roomBuildings.map(building => `<option value="${building.id}">${escapeHtml(building.label)}</option>`).join("")
...
async function loadRooms() {
    ...
    if (!tbody) return;
    tbody.innerHTML = "<tr><td colspan=\"8\" class=\"table-empty\">Đang tải danh sách phòng...</td></tr>";
    ...
    if (!roomBuildings.length) {
        ...
        roomBuildings = Array.from(new Map(
            allRooms.map(room => [room.buildingId, {
                id: room.buildingId,
...
function renderRoomsTable() {
    const tbody = document.getElementById("rooms-table-body");
    if (!tbody) return;
    updatePaginationUi("rooms", paginationState.rooms.totalItems || adminRooms.length);

    if (!adminRooms.length) {
        tbody.innerHTML = "<tr><td colspan=\"8\" class=\"table-empty\">Không có phòng phù hợp bộ lọc hiện tại.</td></tr>";
        return;
    }

    tbody.innerHTML = adminRooms.map(room => `
    ...
    tbody.querySelectorAll("[data-room-view]").forEach(button => {
        button.addEventListener("click", () => selectRoom(Number(button.dataset.roomView)));
    });
}
...
async function selectRoom(roomId) {
    ...
    document.getElementById("room-detail-code").textContent = room.roomCode || "Da chon";
    document.getElementById("room-detail-building").textContent = `${room.buildingName} (${room.buildingCode})`;
    document.getElementById("room-detail-gender").textContent = room.genderAllowed || "-";
    document.getElementById("room-building-id").value = String(room.buildingId || "");
...
function clearRoomDetail() {
    ...
    if (roomBuildings.length) document.getElementById("room-building-id").value = String(roomBuildings[0].id);
    ...
    document.getElementById("room-facilities-list").innerHTML = "<div class=\"empty-state\">Chon mot phong de xem thiet bi.</div>";
    clearFacilityForm();
}
...
async function loadRoomFacilities(roomId) {
    ...
    selectedFacilityId = null;
    const room = adminRooms.find(item => item.id === roomId);
    document.getElementById("facility-room-code").textContent = room?.roomCode || "Da chon phong";
    clearFacilityForm();
    renderRoomFacilities();
}
...
function renderRoomFacilities() {
    ...
    if (!container) return;
    if (!selectedRoomId) {
        container.innerHTML = "<div class=\"empty-state\">Chon mot phong de xem thiet bi.</div>";
        return;
    }
    if (!selectedRoomFacilities.length) {
        container.innerHTML = "<div class=\"empty-state\">Phong nay chua co thiet bi nao.</div>";
        return;
    }
    container.innerHTML = selectedRoomFacilities.map(item => `
    ...
    container.querySelectorAll("[data-facility-view]").forEach(button => {
        button.addEventListener("click", () => {
            const facility = selectedRoomFacilities.find(item => item.id === Number(button.dataset.facilityView));
            if (!facility) return;
...
function getStudentFilters() {
    return {
        keyword: document.getElementById("student-search")?.value.trim().toLowerCase() || "",
...
function bindStudentControls() {
    ...
    document.getElementById("student-search")?.addEventListener("input", rerenderStudents);
    document.getElementById("student-filter-active")?.addEventListener("change", rerenderStudents);
    ...
    document.getElementById("student-form")?.addEventListener("submit", async event => {
        ...
        const permanentAddress = document.getElementById("student-address").value.trim();
        const isActive = document.getElementById("student-is-active").value === "true";
    ...
    document.getElementById("delete-student-btn")?.addEventListener("click", async () => {
        ...
        const res = await callApi(`/students/${selectedStudentId}`, { method: "DELETE" });
        if (res?.ok) {
...
async function loadStudents() {
    ...
    if (!tbody) return;
    tbody.innerHTML = "<tr><td colspan=\"7\" class=\"table-empty\">Đang tải danh sách sinh viên...</td></tr>";
    ...
    if (selectedStudentId) {
        const exists = adminStudents.some(student => student.id === selectedStudentId);
        if (exists) {
            await selectStudent(selectedStudentId);
        } else {
            selectedStudentId = null;
...
function renderStudentsTable() {
    const tbody = document.getElementById("students-table-body");
    if (!tbody) return;
    updatePaginationUi("students", paginationState.students.totalItems || adminStudents.length);

    if (!adminStudents.length) {
        tbody.innerHTML = "<tr><td colspan=\"7\" class=\"table-empty\">Không có sinh viên phù hợp bộ lọc hiện tại.</td></tr>";
        return;
    }

    tbody.innerHTML = adminStudents.map(student => `
    ...
    tbody.querySelectorAll("[data-student-view]").forEach(button => {
        button.addEventListener("click", () => selectStudent(Number(button.dataset.studentView)));
    });
}
...
async function selectStudent(studentId) {
    ...
    document.getElementById("student-detail-name").textContent = student.fullName || "Da chon";
    document.getElementById("student-detail-email").textContent = student.email || "-";
    document.getElementById("student-detail-created").textContent = formatDate(student.createdAt);
    document.getElementById("student-phone").value = student.phone || "";
    document.getElementById("student-address").value = student.permanentAddress || "";
    document.getElementById("student-is-active").value = String(Boolean(student.isActive));
    setStudentError("");
    renderStudentsTable();
}
...
function bindRevenueControls() {
    ...
    if (!document.getElementById("revenue-start-date").value) {
        document.getElementById("revenue-start-date").value = firstDay.toISOString().slice(0, 10);
    }
    if (!document.getElementById("revenue-end-date").value) {
        document.getElementById("revenue-end-date").value = today.toISOString().slice(0, 10);
    }
    document.getElementById("load-revenue-btn")?.addEventListener("click", loadRevenue);
    document.getElementById("export-revenue-btn")?.addEventListener("click", exportRevenue);
}
...
async function loadRevenue() {
    ...
    tbody.innerHTML = "<tr><td colspan=\"9\" class=\"table-empty\">Đang tải báo cáo doanh thu...</td></tr>";
    ...
    if (!res?.ok || !res.data?.data) {
        ...
        tbody.innerHTML = "<tr><td colspan=\"9\" class=\"table-empty\">Không có dữ liệu doanh thu.</td></tr>";
        resetRevenueStats();
        adminRevenueDetails = [];
    ...
    document.getElementById("revenue-room-fee").textContent = formatCurrency(data.totalRoomFee);
    document.getElementById("revenue-electric-fee").textContent = formatCurrency(data.totalElectricFee);
    document.getElementById("revenue-water-fee").textContent = formatCurrency(data.totalWaterFee);
    document.getElementById("revenue-grand-total").textContent = formatCurrency(data.grandTotal);
    document.getElementById("revenue-total-invoices").textContent = data.totalInvoices ?? 0;
    document.getElementById("revenue-paid-invoices").textContent = data.paidInvoices ?? 0;
    document.getElementById("revenue-unpaid-invoices").textContent = data.unpaidInvoices ?? 0;
    adminRevenueDetails = Array.isArray(data.details) ? data.details : [];
    resetPage("revenue");
    renderRevenueTable();
    return;
...
function renderRevenueTable() {
    const tbody = document.getElementById("revenue-table-body");
    if (!tbody) return;
    updatePaginationUi("revenue", adminRevenueDetails.length);

    if (!adminRevenueDetails.length) {
        tbody.innerHTML = "<tr><td colspan=\"9\" class=\"table-empty\">Không có chi tiết doanh thu.</td></tr>";
        return;
    }

    const { items } = getPagedItems("revenue", adminRevenueDetails);
...
function resetRevenueStats() {
    ...
    document.getElementById("revenue-electric-fee").textContent = formatCurrency(0);
    document.getElementById("revenue-water-fee").textContent = formatCurrency(0);
    document.getElementById("revenue-grand-total").textContent = formatCurrency(0);
...
async function loadInvoices() {
    ...
    tbody.innerHTML = "<tr><td colspan=\"11\" class=\"table-empty\">Đang tải danh sách hóa đơn...</td></tr>";
    setInvoiceActionError("");
...
function renderInvoicesTable() {
    const tbody = document.getElementById("invoices-table-body");
    if (!tbody) return;
    updatePaginationUi("invoices", paginationState.invoices.totalItems || adminInvoices.length);

    if (!adminInvoices.length) {
        tbody.innerHTML = "<tr><td colspan=\"11\" class=\"table-empty\">Không có hóa đơn phù hợp bộ lọc hiện tại.</td></tr>";
        return;
    }

    tbody.innerHTML = adminInvoices.map(invoice => `
        <tr>
            ...
            <td>
                <div class="invoice-status-actions">
                    <button type="button" class="secondary-btn" data-invoice-view="${invoice.id}">Chi tiet</button>
                    ${invoice.status === "Unpaid"
                        ? `<button type="button" class="primary-btn" data-invoice-pay="${invoice.id}">Dnh d?u d thu</button>`
                        : ""}
                </div>
            </td>
        </tr>
    ...
    tbody.querySelectorAll("[data-invoice-view]").forEach(button => {
        button.addEventListener("click", async () => {
            const resDetail = await callApi(`/invoices/${button.dataset.invoiceView}`);
            const invoice = resDetail?.data;
            if (!resDetail?.ok || !invoice) {
                adminToast(resDetail?.data?.message || "Không thể lấy chi tiết hóa đơn.", true);
                return;
            }
            ...
            const details = [
                `Hoa Don #${invoice.id}`,
                `Ky: ${invoice.period || "-"}`,
                `Phong: ${invoice.roomCode || "-"}`,
                `Sinh vien: ${invoice.studentName || "-"}`,
                `Tien phong: ${formatCurrency(invoice.roomFee)}`,
                `Tien Dien: ${formatCurrency(invoice.electricFee)}`,
                `Tien nuoc: ${formatCurrency(invoice.waterFee)}`,
                `Tong tien: ${formatCurrency(invoice.totalAmount)}`,
                `Tr?ng thi: ${invoice.status || "-"}`,
                `Ngy pht hnh: ${formatDate(invoice.issuedAt)}`
            ].join("\n");
    ...
    tbody.querySelectorAll("[data-invoice-pay]").forEach(button => {
        button.addEventListener("click", () => withAction(button, async () => {
            const resPay = await callApi(`/invoices/${button.dataset.invoicePay}/pay`, { method: "PUT" });
            if (resPay?.ok) {
                adminToast(resPay.data?.message || "D cap nhat ha don d thanh ton.");
                loadInvoices();
            } else {
                adminToast(resPay?.data?.message || "Khng th? cap nhat tr?ng thi thanh ton.", true);
            }
        }));
    });
}
...
function renderNotificationsList() {
    ...
    if (!container) return;
    updatePaginationUi("notifications", paginationState.notifications.totalItems || adminNotifications.length);
    ...
    container.innerHTML = adminNotifications.map(item => `
        <article class="queue-item">
            <div class="queue-head">
                <strong>${escapeHtml(item.title)}</strong>
